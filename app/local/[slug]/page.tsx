...
export default function PaginaLocal(props: { params: Promise<{ slug: string }> }) {
  ...
  if (cargando) return <div className="p-20 text-center font-bold text-blue-600 animate-pulse uppercase tracking-widest">Cargando Agéndalo...</div>;
  ...
  const linkWhatsApp = `https://wa.me/${local.telefono?.replace(/\D/g, '')}?text=${mensajeWA}`;
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-900 pb-20">
      ...
      <div className="h-64 bg-blue-900 relative flex items-center justify-center text-white">
        ...
        <h1 className="relative text-4xl md:text-6xl font-black uppercase tracking-tighter text-center px-4 drop-shadow-lg">{local.nombre}</h1>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-6">
        <div className="text-center mb-12 -mt-10 relative z-10">
          <div className="bg-white shadow-xl rounded-3xl p-6 border border-gray-100 inline-block max-w-full">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <a href={linkWhatsApp} className="bg-[#25D366] text-white px-5 py-2 rounded-xl font-bold text-sm flex items-center gap-3 shadow-lg hover:scale-105 transition-transform">
        ...
        {/* MAPA SIN ERROR API */}
        {local.maps_url && (
          <div className="mb-12 rounded-[2rem] overflow-hidden border-4 border-gray-50 bg-gray-50 p-10 text-center shadow-sm">
            <h3 className="font-bold text-gray-800 mb-4 text-xl tracking-tight">Encuéntranos en {local.direccion}</h3>
            <a href={local.maps_url} target="_blank" className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-black transition-all inline-block shadow-lg">
        ...
        <div className="grid gap-4">
          {servicios.map(s => (
            <div key={s.id} className="border-2 border-gray-50 p-6 rounded-[2rem] flex justify-between items-center bg-white shadow-sm hover:border-blue-500 transition-all group">
              ...
              <button onClick={() => { setServicioSeleccionado(s); setMostrarForm(true); }} className="bg-black text-white px-8 py-3 rounded-2xl font-bold hover:bg-blue-600 transition shadow-md">Agendar</button>
            </div>
          ))}
        </div>
      </div>
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden max-w-5xl w-full flex flex-col md:flex-row max-h-[90vh]">
            ...
            <div className="p-8 border-r bg-gray-50/50 md:w-1/2 overflow-y-auto">
              <div className="mb-6">
                ...
                <h3 className="text-xl font-bold leading-tight uppercase tracking-tighter">Usted está agendando: <br/><span className="text-blue-600">{servicioSeleccionado.nombre} — ${servicioSeleccionado.precio.toLocaleString('es-CL')}</span></h3>
              </div>
                ...
                <div className="grid grid-cols-4 gap-2 animate-in fade-in slide-in-from-bottom-2">
                  {horarios.map(h => (
                    <button key={h} type="button" disabled={bloquesOcupados.includes(h)} onClick={() => setHoraSeleccionada(h)}
                      className={`p-2 rounded-xl text-[10px] font-black border transition-all ${bloquesOcupados.includes(h) ? 'bg-gray-50 text-gray-200 cursor-not-allowed' : horaSeleccionada === h ? 'bg-black text-white border-black shadow-lg shadow-gray-200' : 'bg-white border-gray-100 hover:border-blue-500'}`}>
                      {h}
                    </button>
                  ))}
                </div>
            ...
            <div className="p-8 md:w-1/2 flex flex-col justify-center bg-white overflow-y-auto">
              {!horaSeleccionada ? (
                <div className="text-center py-20 text-gray-400 font-bold italic tracking-tight">Seleccione fecha y hora para continuar...</div>
              ) : (
                <form onSubmit={enviar} className="space-y-4 animate-in slide-in-from-right-4">
                  ...
                  <input required placeholder="Nombre Completo" className="w-full border p-4 rounded-2xl outline-none bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all" value={nombre} onChange={e => setNombre(e.target.value)} />
                  <input required type="email" placeholder="Correo Electrónico" className="w-full border p-4 rounded-2xl outline-none bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all" value={email} onChange={e => setEmail(e.target.value)} />
                  <input required type="tel" placeholder="WhatsApp" className="w-full border p-4 rounded-2xl outline-none bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all" value={telefonoCliente} onChange={e => setTelefonoCliente(e.target.value)} />
                  ...
                  <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black shadow-xl hover:bg-blue-700 transition-all uppercase tracking-widest text-sm">
                    {metodoPago === 'online' ? 'Ir a Pagar' : 'Confirmar Agenda'}
                  </button>
                  <button type="button" onClick={() => setMostrarForm(false)} className="w-full text-gray-400 text-[10px] font-black pt-4 uppercase tracking-[0.2em] text-center hover:text-red-500 transition-colors">Cerrar</button>
                </form>
              )}
            </div>
          </div>
