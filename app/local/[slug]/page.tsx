...
export default function PaginaLocal(props: { params: Promise<{ slug: string }> }) {
  ...
  if (cargando) return <div className="p-20 text-center font-bold text-blue-600">Cargando Agéndalo...</div>;
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-900 pb-20">
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        {local.maps_url && (
          <div className="mb-12 rounded-[2rem] overflow-hidden border-4 border-gray-50 bg-gray-50 p-8 text-center">
            <h3 className="font-bold text-gray-800 mb-4">Ubicación de {local.nombre}</h3>
            <a href={local.maps_url} target="_blank" className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-black transition-colors inline-block">
            ...
            <div className="p-8 border-r bg-gray-50/50 md:w-1/2 overflow-y-auto">
              ...
              <div className="bg-white p-6 rounded-3xl border mb-6">
                ...
                <div className="grid grid-cols-7 gap-1 text-center">
                  {['D','L','M','M','J','V','S'].map(d => <div key={d} className="text-[10px] font-bold text-gray-300">{d}</div>)}
                  {dias.map(dia => {
            ...
            <div className="p-8 md:w-1/2 flex flex-col justify-center bg-white overflow-y-auto">
              {!horaSeleccionada ? (
                ...
                <form onSubmit={enviar} className="space-y-4">
                  ...
                  <input required type="tel" placeholder="WhatsApp" className="w-full border p-4 rounded-2xl outline-none bg-gray-50" value={telefonoCliente} onChange={e => setTelefonoCliente(e.target.value)} />
                  ...
                  <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black shadow-xl hover:bg-blue-700 transition uppercase tracking-widest">
                    {metodoPago === 'online' ? 'Ir a Pagar' : 'Confirmar Agenda'}
                  </button>
                  <button type="button" onClick={() => setMostrarForm(false)} className="w-full text-gray-400 text-xs font-bold pt-2 uppercase text-center">Cerrar</button>
                </form>
              )}
            </div>
          </div>
