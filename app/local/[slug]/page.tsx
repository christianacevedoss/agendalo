...
interface Local {
  nombre: string;
  foto_banner: string;
  maps_url?: string; // Nombre exacto de tu variable en la tabla
  telefono?: string;
  direccion?: string;
  descripcion?: string;
}
...
export default function PaginaLocal(props: { params: Promise<{ slug: string }> }) {
  ...
  const linkWhatsApp = `https://wa.me/${local.telefono?.replace(/\s+/g, '')}?text=${mensajeWA}`;
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-900 pb-20">
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="text-center mb-12 -mt-10 relative z-10">
          <div className="bg-white shadow-xl rounded-3xl p-6 border border-gray-100 inline-block max-w-full">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              {/* Botón WhatsApp con Logo y Mensaje */}
              <a 
                href={linkWhatsApp} 
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-[#25D366] text-white px-4 py-2 rounded-xl font-bold text-sm flex items-center gap-2 shadow-lg shadow-green-100 hover:scale-105 transition-transform"
              >
        ...
        {/* MAPA DE GOOGLE INTEGRADO CORREGIDO */}
        {local.maps_url && (
          <div className="mb-12 rounded-[2rem] overflow-hidden border-4 border-gray-50 shadow-inner h-80 w-full relative">
            {/* Si es una URL de compartir (maps.app.goo.gl o google.com/maps), el iframe necesita ser embebido. 
                Como medida segura, mostramos el mapa interactivo de Google a través de su buscador. */}
            <iframe 
              width="100%" 
              height="100%" 
              style={{ border: 0 }} 
              ...
              // NOTA: Si no tienes API KEY aún, usaremos un link visual y el botón de abajo.
            ></iframe>
            <div className="absolute inset-0 bg-gray-100 flex items-center justify-center -z-10 text-gray-400 font-bold">Cargando mapa...</div>
            
            <div className="bg-white p-4 text-center border-t">
              <a href={local.maps_url} target="_blank" rel="noopener noreferrer" className="bg-blue-600 text-white px-6 py-2 rounded-xl font-bold text-xs uppercase tracking-widest inline-block">
      ...
      {/* MODAL... (mantiene el código anterior de dos columnas y pagos) */}
      {/* (Omitido por brevedad, pero asegúrate de mantenerlo igual) */}
    </main>
  );
}