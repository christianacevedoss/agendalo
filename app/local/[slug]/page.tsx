...
interface Local {
  ...
  foto_banner: string;
  maps_url?: string;
  telefono_local?: string; // Corregido según tu base de datos
  direccion?: string;
  descripcion?: string;
}
...
export default function PaginaLocal(props: { params: Promise<{ slug: string }> }) {
  ...
  const [telefonoInput, setTelefonoInput] = useState(''); // Estado local del input
  ...
  const enviar = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('agendamientos').insert([{
      ...
      cliente_email: email, 
      cliente_telefono: telefonoInput, // Corregido según tu base de datos
        ...
        <div className="text-center mb-12 -mt-10 relative z-10">
          <div className="bg-white shadow-xl rounded-3xl p-6 border border-gray-100 inline-block max-w-full">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <a href={linkWhatsApp} target="_blank" className="bg-[#25D366] text-white px-5 py-2 rounded-xl font-bold text-sm flex items-center gap-3 shadow-lg hover:scale-105 transition-transform">
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden max-w-5xl w-full flex flex-col md:flex-row max-h-[90vh]">
            <div className="p-8 border-r bg-gray-50/50 md:w-1/2 overflow-y-auto">
              <div className="mb-6">
                ...
                <h3 className="text-xl font-bold leading-tight uppercase">Usted está agendando: <br/><span className="text-blue-600">{servicioSeleccionado.nombre} — ${servicioSeleccionado.precio.toLocaleString('es-CL')}</span></h3>
              </div>
                  ...
                  {dias.map(dia => {
                    ...
                    return (
                      <button key={dia.toString()} type="button" disabled={esPasado} onClick={() => {setDiaSeleccionado(dia); setHoraSeleccionada('');}}
                        className={`h-9 w-9 rounded-xl text-xs font-bold transition-all ${esPasado ? 'text-gray-200 cursor-not-allowed' : sel ? 'bg-blue-600 text-white' : 'hover:bg-blue-50 text-gray-700'}`}>
                        {format(dia, 'd')}
                      </button>
                    )
                  })}
