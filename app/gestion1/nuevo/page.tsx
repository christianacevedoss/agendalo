...
export default function NuevoCliente() {
  ...
  const [subiendoImagen, setSubiendoImagen] = useState(false) // Estado para carga de imagen
  ...
  const getFechaChile = () => {
    ...
    const offsetChile = ahora.getTimezoneOffset() * 60000; 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payloadDueño = {
      ...
      insert_by: user?.email || 'Admin',
      time_insert: fechaChile
    }

    try {
      const { error: errDueño } = await supabase.from('dueño_local').insert([payloadDueño])
      ...
      const payloadLocal = {
        ...
        foto_banner: form.foto_banner, // Aquí va la URL generada por Supabase
        descripcion: form.descripcion
      }

      const { error: errLocal } = await supabase.from('locales').insert([payloadLocal])
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-12 font-sans text-gray-900 relative">
      ...
      {exito && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-white rounded-[3rem] p-10 max-w-md w-full text-center shadow-2xl animate-bounce-in">
                ...
                <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-2">¡Registro Exitoso!</h2>
                <p className="text-gray-500 font-medium mb-8">El dueño y el local han sido creados correctamente.</p>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-10">
          <div>
            <h1 className="text-3xl font-black italic uppercase tracking-tighter">Nuevo Cliente</h1>
            <p className="text-gray-400 font-bold text-xs uppercase tracking-widest">Crear Dueño + Local</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-blue-50/50 p-8 rounded-[3rem] border border-blue-100 space-y-6">
            ...
            <div className="space-y-1">
              ...
              <input required className="w-full bg-white p-4 rounded-2xl font-bold text-sm outline-none border border-transparent focus:border-blue-300 transition-all" 
                value={form.nombre_dueño} onChange={e => setForm({...form, nombre_dueño: e.target.value})} placeholder="Ej: Juan Pérez" />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input required type="email" className="w-full bg-white p-4 rounded-2xl font-bold text-sm outline-none border border-transparent focus:border-blue-300 transition-all" 
                value={form.correo_dueño} onChange={e => setForm({...form, correo_dueño: e.target.value})} placeholder="juan@gmail.com" />
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input required className="w-full bg-white pl-24 pr-5 py-4 rounded-2xl font-bold text-sm outline-none border border-transparent focus:border-blue-300 transition-all" 
                  value={form.telefono_dueño} onChange={e => setForm({...form, telefono_dueño: cleanPhone(e.target.value)})} placeholder="12345678" />
              </div>
            </div>
                  ...
                  <select className="w-full bg-white p-4 rounded-2xl font-bold text-sm outline-none" value={form.plan} onChange={e => setForm({...form, plan: e.target.value})}>
                    ...
                    {planes.map(p => (
                        <option key={p.id} value={p.nombre}>
                            {p.nombre} {p.valor ? `- $${p.valor.toLocaleString('es-CL')}` : ''}
                        </option>
                    ))}
                  </select>
               </div>
            ...
            {planSeleccionado && (
                <div className="bg-white p-6 rounded-[2rem] border border-blue-100 animate-fade-in mt-4 shadow-sm">
                    ...
                    <p className="text-xs text-gray-500 font-medium leading-relaxed italic">
                        {planSeleccionado.descripcion || "Sin descripción disponible."}
                    </p>
                </div>
            )}
          </div>
          ...
          <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-xl space-y-6">
            ...
            {/* FILA 1: NOMBRE LOCAL + EMAIL LOCAL */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <input type="email" className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none" 
                    value={form.email_local} onChange={e => setForm({...form, email_local: e.target.value})} placeholder="Opcional" />
                </div>
            </div>

            {/* FILA 2: RUTA (2/3) + TELÉFONO LOCAL (1/3) */}
                ...
                <div className="md:col-span-2 space-y-1">
                    ...
                    <div className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm text-blue-600 flex items-center gap-1 border border-transparent focus-within:border-blue-200 transition-all overflow-hidden">
                      ...
                      <input required className="bg-transparent outline-none flex-1 placeholder-blue-200 w-full min-w-0" 
                        value={form.slug} onChange={e => setForm({...form, slug: e.target.value})} placeholder="ruta" />
                    </div>
                </div>
                ...
                <div className="md:col-span-1 space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input className="w-full bg-gray-50 pl-16 pr-3 py-4 rounded-2xl font-bold text-sm outline-none" 
                      value={form.telefono_local} onChange={e => setForm({...form, telefono_local: cleanPhone(e.target.value)})} placeholder="Opcional" />
                  </div>
               </div>
            </div>
            ...
            <div className="space-y-2">
               <label className="text-[10px] font-black text-gray-400 uppercase ml-2">Rubro (Ayuda a encontrar tu página)</label>
            ...
            <div className="space-y-1">
              ...
              <input className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none" 
                value={form.direccion} onChange={e => setForm({...form, direccion: e.target.value})} placeholder="Opcional: Calle, Número, Comuna" />
            </div>

            {/* FILA 5: EXTRAS */}
            <div className="pt-4 border-t border-gray-100 space-y-4">
               
               {/* MAPS + BANNER (CAMBIADO A FILE INPUT) */}
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <div className="space-y-1">
                   ...
                   <div className="space-y-1">
                      <label className="text-[10px] font-black text-gray-400 uppercase ml-2">
                          Foto Portada {subiendoImagen && <span className="text-blue-500 animate-pulse">- Subiendo...</span>}
    ...
    (Ideal: 1200x675px o formato horizontal)
               ...
               {form.foto_banner && (
                 <div className="w-full h-32 rounded-xl overflow-hidden relative group border border-gray-200">
               ...
               <div className="space-y-1">
                  ...
                  <textarea className="w-full bg-gray-50 p-3 rounded-xl font-medium text-xs outline-none h-20 resize-none" 
                            placeholder="Opcional: Descripción del negocio..." 
            ...
            <div className="bg-gray-50 p-4 rounded-2xl flex items-center justify-between border border-gray-100">
                <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-gray-400 font-medium">Si desactivas, el local quedará en Standby.</p>
                </div>
          ...
          <div className="lg:col-span-2 pb-10">
            <button disabled={enviando || subiendoImagen} type="submit" className="w-full bg-black text-white font-black py-6 rounded-[2rem] shadow-2xl hover:bg-gray-800 transition-all uppercase tracking-widest text-sm disabled:opacity-50 disabled:cursor-not-allowed">
              {enviando ? 'Guardando...' : subiendoImagen ? 'Esperando imagen...' : 'Confirmar Creación'}
            </button>
          </div>
